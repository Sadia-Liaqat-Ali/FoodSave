...
mysqli_query($conn, "
    ...
    WHERE id='$aid' AND driver_id='$driver_id'
");

// Update donation_requests (ONLY status)
mysqli_query($conn, "
...
<li class="nav-item">
    <a class="nav-link text-white" href="upload_receipt.php">
        <i class="fas fa-truck me-2"></i>Upload Receipt
    </a>
</li>
...
<div class="row mb-4">
    <div class="col-md-4">
        <div class="card text-white bg-primary mb-3">
            <div class="card-body">
                <h5 class="card-title">Total Duties</h5>
                <p class="card-text fs-3"><?= $total_assigned; ?></p>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card text-white bg-success mb-3">
            <div class="card-body">
                <h5 class="card-title">Completed Duties</h5>
                <p class="card-text fs-3"><?= $total_completed; ?></p>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card text-white bg-warning mb-3">
            <div class="card-body">
                <h5 class="card-title">Pending Duties</h5>
...
<td>
    <?php if ($row['duty_status'] != 'completed'): ?>
        <a href="?complete_id=<?= $row['assignment_id']; ?>" class="btn btn-sm btn-success mb-1">
            <i class="fas fa-check"></i>
        </a>
    <?php else: ?>
        <!-- Generate Receipt button for completed duties -->
        <a href="pickup_receipt.php?rid=<?= $row['request_id']; ?>" target="_blank" 
           class="btn btn-sm btn-primary mb-1">
           <i class="fas fa-file-invoice"></i> Receipt
        </a>
    <?php endif; ?>

    <a href="?delete_id=<?= $row['assignment_id']; ?>" onclick="return confirm('Delete this duty?')" class="btn btn-sm btn-danger mb-1">
